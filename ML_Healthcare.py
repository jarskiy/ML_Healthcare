...
tit1.markdown("<h1 style='text-align: center;'><u>Machine Learning in Healthcare</u> </h1>",
              unsafe_allow_html=True)
tit2.image("healthcare2.png")
st.sidebar.title("Dataset and Classifier")

dataset_name = st.sidebar.selectbox(
    "Select Dataset: ", ('Heart Attack', "Breast Cancer"))
...
def compute(Y_pred, Y_test):
    ...
    plt.scatter(range(len(Y_pred)), Y_pred,
                color="yellow", lw=5, label="Predictions")
...
def user_inputs_ui(dataset_name, data):
    ...
    if dataset_name == "Breast Cancer":
        ...
        for col in X.columns:
            name = col
            col = st.number_input(col, abs(
                X[col].min()-round(X[col].std())), abs(X[col].max()+round(X[col].std())))
